...
def create_sheet_1(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    territories = ['US',
    'UK',
    'GX',
    'CH',
    ...
    for idx, d in enumerate(hits, start=1):
        ...
        if d["jcr:path"] in mc[0]:
            mc[1].append([d["jcr:path"],''])
...
def create_sheet_2(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    ...
    for idx, d in enumerate(hits, start=1):
        ...
        ws.cell(row=row, column=data_col, value=d["jcr:path"])
        if d["jcr:path"] in mc[0]:
            mc[1].append([d["jcr:path"],'Ready for processing'])
...
def create_sheet_3(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    ...
    for idx, d in enumerate(hits, start=1):
        ...
        ws.cell(row=row, column=data_col, value=d["jcr:path"])
        if d["jcr:path"] in mc[0]:
            mc[1].append([d["jcr:path"],'Processed'])
...
def create_sheet_4(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    ...
    for idx, d in enumerate(hits, start=1):
        ...
        ws.cell(row=row, column=data_col, value=d["jcr:path"])
        if d["jcr:path"] in mc[0]:
            mc[1].append([d["jcr:path"],'Submitted'])
...
def create_sheet_5(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    ...
    for idx, d in enumerate(hits, start=1):
        ...
        ws.cell(row=row, column=data_col, value=d["jcr:path"])
        if d["jcr:path"] in mc[0]:
            mc[1].append([d["jcr:path"],'Completed-Banned-Words'])
...
def mc_list():
    URL = f"https://dpe.pwc.com/bin/querybuilder.json?1_property=marketingConsentType&1_property.value=Explicit%20opt-out&p.hits=selective&p.limit=-1&p.properties=jcr%3apath%20status&path=%2fcontent%2fusergenerated%2fcontent%2fpwc%2fus%2fen"
    res=[]
    resp = requests.get(URL, auth=AUTH)
    resp.raise_for_status()
    hits = resp.json().get("hits", [])
    for i in hits:
        #print(i)
        res.append(i['jcr:path'])
    return res
    
def create_sheet_6(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    today_mc = []
    for i in mc[1]:
        if f"{today_year}/{today_month}/{today_date}" in i[0]:
            today_mc.append(i)
    # compute counts
    total_count = len(today_mc)
    
    # create workbook & sheet
    ws = wb.create_sheet(title="Sheet6")
    
    # 1st row, 1st col
    ws.cell(row=1, column=1, value="6. mc Ex Opt Out Failures :")
    
    # 2nd row, 1st col with the query (inject date)
    q1 = "path=/content/usergenerated/content/pwc/us/en"
    q2 = "1_property=marketingConsentType"
    q3 = "1_property.value=Explicit opt-out"
    q4 = "p.limit=-1"
    ws.cell(row=2, column=1, value=q1)
    ws.cell(row=2+1, column=1, value=q2)
    ws.cell(row=2+2, column=1, value=q3)
    ws.cell(row=2+3, column=1, value=q4)
    
    # 7th row
    ws.cell(row=4+3, column=1, value="Result :  -")
    ws.cell(row=4+3, column=2, value=f"{total_count}")
    ws.cell(row=4+3, column=3, value="Payloads")
    ws.cell(row=4+3, column=4, value="Status")

    # Starting at 9th row, 3rd column, write each path and sequence
    start_row = 6+3
    seq_col = 1
    data_col = 3
    
    for idx, d in enumerate(today_mc, start=1):
        row = start_row + idx - 1
        ws.cell(row=row, column=seq_col, value=idx)
        ws.cell(row=row, column=data_col, value=d[0])
        ws.cell(row=row, column=data_col+1, value=d[1])
    # Save
    wb.save(excel_filename)

    print(f"Sheet6 written to {excel_filename}")
    
    print("Sheet6 processed successfully") 
    return f'''
        6. mc Ex Opt Out Failures:

        path=/content/usergenerated/content/pwc/us/en
        1_property=marketingConsentType
        1_property.value=Explicit opt-out
        p.limit=-1

        Result :  {total_count} forms.\n\n
        '''

def create_sheet_7(excel_filename="paths.xlsx", wb=Workbook(), mc=[]):
    today_mc = []
    URL = f"https://dpe.pwc.com/bin/querybuilder.json?1_property=marketingConsentType&1_property.value=Explicit%20opt-out&p.hits=selective&p.limit=-1&p.properties=jcr%3apath%20status&path=%2fcontent%2fusergenerated%2fcontent%2fpwc%2fus%2fen"
    resp = requests.get(URL, auth=AUTH)
    resp.raise_for_status()
    hits = resp.json().get("hits", [])
    for i in hits:
        if f"{today_year}/{today_month}/{today_date}" in i['jcr:path']:
            today_mc.append([i['jcr:path'],i['status']])
    # compute counts
    total_count = len(today_mc)
    ...
    # 1st row, 1st col
    ws.cell(row=1, column=1, value="6. mc Ex Opt Out Failures (for today only) :")
    ...
    print("Sheet6 processed successfully") 
    return f'''
        6. mc Ex Opt Out Failures (for today only) :
...
email_subject = f"Stuck Form Processing Report - {today_date:02d}/{today_month:02d}/{today_year}"
...
mc.append(mc_list())
mc.append([])
email_body+=create_sheet_1(excel_filename=excel_name, wb=wb, mc=mc)
email_body+=create_sheet_2(excel_filename=excel_name, wb=wb, mc=mc)
email_body+=create_sheet_3(excel_filename=excel_name, wb=wb, mc=mc)
email_body+=create_sheet_4(excel_filename=excel_name, wb=wb, mc=mc)
email_body+=create_sheet_5(excel_filename=excel_name, wb=wb, mc=mc)
email_body+=create_sheet_7(excel_filename=excel_name, wb=wb, mc=mc)
...
recipients = [
    ...
    "moumi.banerjee@pwc.com",
    "abhishek.mahato@pwc.com"
]
