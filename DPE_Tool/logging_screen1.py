...
if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
    ...
    BASE_SCRIPT_PATH = os.path.dirname(os.path.abspath(__file__))
    BASIC_CONFIG_FILE = os.path.join(BASE_SCRIPT_PATH, "configfiles", "basicconfig.json")
...
class ViewpointDataCleaner:
    ...
    def create_label_frame(self):
        try:
            self.datalabelframe = LabelFrame(self.vp_metadata_cleanup_ui, text="Enter Details")
            self.databuttonframe = LabelFrame(self.vp_metadata_cleanup_ui)
            self.data_log_frame = LabelFrame(self.vp_metadata_cleanup_ui, text="Logs")
            
            self.datalabelframe.grid( row=1, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="ew")
            self.databuttonframe.grid(row=2, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="ew")
            self.data_log_frame.grid( row=3, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")

            self.vp_metadata_cleanup_ui.grid_columnconfigure(0, weight=1)
            self.vp_metadata_cleanup_ui.grid_columnconfigure(1, weight=3)
            self.vp_metadata_cleanup_ui.grid_rowconfigure(3, weight=1)
    ...
    def ipchange(self, value):
        try:
            ...
            configdata = edcfg.readConfig(CONFIG_FILE)
            if value.lower() == "ip":
                self.environment_frame.grid_rowconfigure(1, weight=0)
                self.ipenter.grid(row=1, column=0, columnspan=2, padx=10,pady=5, sticky="nsew")
                self.varuserent.set("")
                self.varpassent.set("")
            else:
                ...
                # self.environment_frame.grid_columnconfigure(1, weight=3)
                ...
                self.varuserent.set(
                    basicconfigdata.get("vp_"+str(selected_env)+"_username",""))
    ...
    def loginandenv(self):
        ...
        self.varenvdata.set("PRODUCTION")
        self.varipdata = StringVar()
        selected_env = self.varenvdata.get().lower()
        self.decrypted_passwd = GenericFunctions.decrypt_passwd(
            basicconfigdata.get("vp_"+str(selected_env)+"_passwd","")) if basicconfigdata.get("vp_"+str(selected_env)+"_passwd","").strip() != "" else basicconfigdata.get("vp_"+str(selected_env)+"_passwd","").strip()
        self.varuserent.set(basicconfigdata.get("vp_"+str(selected_env)+"_username",""))
        self.varpassent.set(self.decrypted_passwd)
        
        self.varenvdata.trace("w", lambda *args: self.ipchange(self.varenvdata.get()))
        self.varipdata.trace("w", lambda *args: self.checkipdata(self.varipdata))

        self.vp_metadata_cleanup_ui.grid_rowconfigure(1, weight=2)

        self.login_frame = LabelFrame(self.vp_metadata_cleanup_ui, text="Login Details")
        self.environment_frame = LabelFrame(self.vp_metadata_cleanup_ui, text="Environment Details")
              
         # UserName entry
        self.userlabel = ttk.Label(self.login_frame, text="VP Username*", font=(FONT_NAME, FONT_SIZE - 2), anchor=CENTER,)
        ...
        # Password Entry
        self.passlabel = ttk.Label(self.login_frame, text="VP password*", font=(FONT_NAME, FONT_SIZE - 2), anchor=CENTER,)
        ...
        ##Environment Details
        self.envlabel = ttk.Label(self.environment_frame, text="Select Environment*", font=(FONT_NAME, FONT_SIZE - 2), anchor=CENTER,)
        ...
        self.environment_frame.grid_rowconfigure(0, weight=1)
        self.environment_frame.grid_rowconfigure(1, weight=0)
        self.ipenter = ttk.Entry(self.environment_frame, textvariable=self.varipdata)

        self.environment_frame.grid(row=0, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
        self.login_frame.grid(row=0, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="ew")
    ...
    def mainui_design(self):
        try:
            ...
            self.title_label = ttk.Label(self.datalabelframe, text="Cleanup the Meta Data for Viewpoint", anchor=CENTER, style="titleLabel.TLabel", font=(FONT_NAME, FONT_SIZE + 4, "bold"))
            self.title_label.grid(row=0, column=0, columnspan=4, padx=5, pady=5, ipadx=15, ipady=15, sticky="nsew")
            self.datalabelframe.grid_columnconfigure(3,weight=1)

            # # Select label Frame:

            self.selectionlabelframe = LabelFrame(self.datalabelframe, text="Select the source & Enter Details")
            self.selectionlabelframe.grid(row=1, column=0, columnspan=4, padx=5, pady=5, ipadx=5, ipady=5, sticky="nsew")
            self.datalabelframe.grid_rowconfigure(2,weight=1)
            ...
            self.operation_combobox.grid(row=2, column=0, padx=5, pady=5, ipadx=5, ipady=5, sticky="nsew")
            
            self.file_select_querylabel = ttk.Label(self.selectionlabelframe, text="Browse & Select Excel File..",textvariable=self.varexcelfile, font=(FONT_NAME, FONT_SIZE - 2),)
            self.file_select_querylabel.grid(row=2, column=1, columnspan=2, padx=5, pady=5, ipadx=5, ipady=5, sticky="nsew")
            
            self.file_select_btn = ttk.Button(self.selectionlabelframe, text="Select File", command=self.open_file)
            self.file_select_btn.grid(row=2, column=3, padx=5, pady=5, ipadx=5, ipady=5, sticky="nsew")
