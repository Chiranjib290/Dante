...
from clearDPEInboxItems1 import ClearDPEInboxItems
...
from logging_screen1 import LoggingScreen, ViewpointDataCleaner
...
class DPEInboxClearing:
    def __init__(self, master):
        ...
        self.master.title(APPLICATION_NAME + " - " + "DPE Dashboard")
    ...
    def createmenubar(self):
        try:
            ...
            self.inbox_menu.add_command(label="Retrieve inbox data", command=self.queryScreen)
...
class BasicConfigWindow:
    ...
    def vp_enable_disable_input(self, data):
        try:
            if data == 0:
                self.save_vp_username_entry["state"] = "disabled"
                self.save_vp_password_entry["state"] = "disabled"
                self.vp_env_details_combobox["state"] = "disabled"
            elif data == 1:
                self.save_vp_username_entry["state"] = "normal"
                self.save_vp_password_entry["state"] = "normal"
                self.vp_env_details_combobox["state"] = "readonly"

        except:
            logger.error("Below Exception occured: ", exc_info=True)

    
    def selected_app(self, data):
        try:
            for child in self.save_user_details_frame.winfo_children():
                if child.winfo_ismapped():
                    child.grid_forget()
            
            if data.strip().lower() == "dpe":
                self.save_user_details_checkbtn.grid(row=0, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                
                self.env_details_combobox.grid(row=0, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                
                self.save_username_label.grid(row=1, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                self.save_username_entry.grid(row=1, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")

                self.save_password_label.grid(row=2, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                self.save_password_entry.grid(row=2, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
            
            elif data.strip().lower() == "viewpoint":
                self.save_vp_user_details_checkbtn.grid(row=0, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                
                self.vp_env_details_combobox.grid(row=0, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                
                self.save_vp_username_label.grid(row=1, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                self.save_vp_username_entry.grid(row=1, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")

                self.save_vp_password_label.grid(row=2, column=0, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                self.save_vp_password_entry.grid(row=2, column=1, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
    ...
    def basicconfigmaindesign(self):
        try:
            ...
            self.decrypted_vp_passwd = GenericFunctions.decrypt_passwd(
                basicconfigdata.get("vp_production_passwd")) if basicconfigdata.get("vp_production_passwd","").strip() != "" else basicconfigdata.get("vp_production_passwd","").strip()
            ...
            self.save_vp_user_details_var = IntVar()
            self.save_vp_user_details_var.set(
                int(self.basicdata.get("save_vp_user_details",False)))
            
            self.var_save_username = StringVar()
            self.var_save_username.set(self.basicdata.get("production_username",""))

            self.var_save_vp_username = StringVar()
            self.var_save_vp_username.set(self.basicdata.get("vp_production_username",""))
            
            self.var_save_password = StringVar()
            self.var_save_password.set(self.decrypted_passwd)

            self.var_save_vp_password = StringVar()
            self.var_save_vp_password.set(self.decrypted_vp_passwd)
            
            self.env_details_var = StringVar()
            self.env_details_var.set("Production")

            self.vp_env_details_var = StringVar()
            self.vp_env_details_var.set("Production")

            self.selected_app_var = StringVar()
            self.selected_app_var.set("DPE")
            ...
            self.save_vp_user_details_var.trace(
                "w", lambda *args: self.vp_enable_disable_input(
                    self.save_vp_user_details_var.get())
            )

            self.selected_app_var.trace(
                "w", lambda *args: self.selected_app(
                    self.selected_app_var.get())
            )
            ...
            self.selected_app_label = ttk.Label(
                self.frame_1,
                text="Select Application",
                font=(FONT_NAME, FONT_SIZE - 2),
                anchor=CENTER,
            )
            self.selected_app_label.grid(
                row=1, column=0, padx=20, pady=20, ipadx=10, ipady=10, sticky="nsew"
            )
            self.frame_1.grid_columnconfigure(0, weight=1)
            ...
            self.selected_app_entry = ttk.OptionMenu(
                self.frame_1, self.selected_app_var, *self.selected_app_data
            )
            self.selected_app_entry.grid(
                row=1, column=1, padx=20, pady=20, ipadx=10, ipady=10, sticky="nsew"
            )
            #self.frame_1.grid_columnconfigure(1, weight=1)
            ...
            self.save_user_details_frame.grid(
                row=2, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
            #self.frame_1.grid_columnconfigure(0, weight=1)



            self.save_user_details_checkbtn = ttk.Checkbutton(
                self.save_user_details_frame, text="Save DPE User Deatils", variable=self.save_user_details_var, onvalue=1, offvalue=0)
            ...
            self.save_vp_user_details_checkbtn = ttk.Checkbutton(
                self.save_user_details_frame, text="Save VP User Deatils", variable=self.save_vp_user_details_var, onvalue=1, offvalue=0)
            
            vp_env_values = configdata.get("environments", []).copy()
            self.vp_env_details_combobox = ttk.Combobox(
                self.save_user_details_frame, textvariable=self.vp_env_details_var, state="readonly", values=vp_env_values)

            self.save_vp_username_label = ttk.Label(
                self.save_user_details_frame, text="VP Username", font=(FONT_NAME, FONT_SIZE-2), anchor=CENTER)
            
            self.save_vp_username_entry = ttk.Entry(
                self.save_user_details_frame, textvariable=self.var_save_vp_username, style="BW.TLabel")

            self.save_vp_password_label = ttk.Label(
                self.save_user_details_frame, text="VP Password", font=(FONT_NAME, FONT_SIZE-2), anchor=CENTER)
            self.save_vp_password_entry = ttk.Entry(
                self.save_user_details_frame, show="*", textvariable=self.var_save_vp_password, style="BW.TLabel")
            
            
            #self.erro_show_label = ttk.Label(self.save_user_details_frame, text="", font=(FONT_NAME, FONT_SIZE-2), anchor=CENTER, style="BWDanger.TLabel")

            #self.vp_erro_show_label = ttk.Label(self.save_user_details_frame, text="", font=(FONT_NAME, FONT_SIZE-2), anchor=CENTER, style="BWDanger.TLabel")

            self.selected_app(self.selected_app_var.get())
            
            self.enable_disable_input(self.save_user_details_var.get()) 
   
            self.vp_enable_disable_input(self.save_vp_user_details_var.get())
               

            # Combox Event-------------------------------------------------------------------
            self.env_details_combobox.bind("<<ComboboxSelected>>", self.change_environment)
            self.vp_env_details_combobox.bind("<<ComboboxSelected>>", self.change_vp_environment)
    ...
    def change_vp_environment(self, event):
        try:
            selected_env = self.vp_env_details_var.get().lower()
            self.var_save_vp_username.set(
                self.basicdata.get("vp_"+selected_env+"_username","")
                )
            self.decrypted_vp_passwd = GenericFunctions.decrypt_passwd(
                basicconfigdata.get("vp_"+str(selected_env)+"_passwd","")) if basicconfigdata.get("vp_"+str(selected_env)+"_passwd","").strip() != "" else basicconfigdata.get("vp_"+str(selected_env)+"_passwd","").strip()
            self.var_save_vp_password.set(self.decrypted_vp_passwd)
    ...
    def savesettings(self):
        try:
            ...
            selected_vp_environment = self.vp_env_details_var.get().lower()
            can_be_closed = False
            ...
            elif(self.save_user_details_var.get() == 1 and self.selected_app_var.get().strip().lower() == "dpe"):
                if(self.var_save_username.get().strip() != "" and self.var_save_password.get().strip() != ""):
                    self.basicdata[str(selected_environment)+"_username"] = self.var_save_username.get().strip()
                    self.basicdata[str(selected_environment)+"_passwd"] = GenericFunctions.encrypt_passwd(self.var_save_password.get().strip())
                else:
                    errorfields = []
                    ...
                    #self.erro_show_label.grid(row=3, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                    #self.erro_show_label.config(text="Below fields are Mandatory\n"+",".join(errorfields))


            if(self.save_vp_user_details_var.get() == 0):
                self.basicdata["username"] = ""
                self.basicdata["passwd"] = ""
                for env in env_list:
                    self.basicdata[f"vp_{env.lower()}_username"] = ""
                    self.basicdata[f"vp_{env.lower()}_passwd"] = ""
            
            elif(self.save_vp_user_details_var.get() == 1 and self.selected_app_var.get().strip().lower() == "viewpoint"):
                if(self.var_save_vp_username.get().strip() != "" and self.var_save_vp_password.get().strip() != ""):
                    self.basicdata["vp_"+str(selected_vp_environment)+"_username"] = self.var_save_vp_username.get().strip()
                    self.basicdata["vp_"+str(selected_vp_environment)+"_passwd"] = GenericFunctions.encrypt_passwd(self.var_save_vp_password.get().strip())
                else:
                    errorfields = []
                    if(self.var_save_vp_username.get().strip() == ""):
                        errorfields.append("VP Username")
                    if(self.var_save_vp_password.get().strip() == ""):
                        errorfields.append("VP Password")
                    canSave = False
                    messagebox.showerror("Empty Mandatory Fields","Below fields are Mandatory\n"+",".join(errorfields),parent=self.basicconfigwin)
                    #self.vp_erro_show_label.grid(row=3, column=0, columnspan=2, padx=10, pady=10, ipadx=10, ipady=10, sticky="nsew")
                    #self.vp_erro_show_label.config(text="Below fields are Mandatory\n"+",".join(errorfields))
                    
            if canSave:
                self.basicdata["save_user_details"] = self.save_user_details_var.get()
                self.basicdata["save_vp_user_details"] = self.save_vp_user_details_var.get()
                self.basicdata["selected theme"] = self.varthemedata.get().lower().strip()
                _config_saved = edcfg.updateConfig(self.basicdata, BASIC_CONFIG_FILE)
                basicconfigdata = self.basicdata

                can_be_closed = messagebox.askyesnocancel(
                    "Please Confirm", "Data has been Saved Successfully.\nDo you want to Close the settings?"
                    , parent=self.basicconfigwin)
...
class QueryBuilder:
    def __init__(self, master):
        ...
        self.querybuilderwin.excelfile = ""
    ...
    def querywindowmaindesign(self):
        try:
            ...
            self.selectallbtn.pack(side="left", anchor="w", padx=10, pady=0)
            ...
            self.selectnonebtn.pack(side="left", anchor="w", padx=0, pady=0)
            
            # Error Label
            self.error_label = Label(self.smallbtnframe, text="", font=(FONT_NAME, LOCAL_FONT_SIZE),)
            self.error_label.pack(side="left", anchor="w", padx=0, pady=0)
    ...
    def getQueryData(self, issue_type, ip, uname, pwd, cntry, varpath, lbound, ubound, status):
        ...
        try:
            ...
            if issue_type == "form processing":
                ...
                if issue_type == "obscene language" and self.querybuilderwin.excelfile != "":
                    filename, ext = os.path.splitext(self.querybuilderwin.excelfile)
                    if(ext == ".xlsx" or ext == ".xls" or ext == ".xlsm"):
                        paylist = self.adderandremoverlist(self.querybuilderwin.excelfile)
                    else:
                        messagebox.showerror(
                            "Invalid File Type"
                        )
                ...
                self.retrieved_data = self.failed_data.retrieve_list(
                    issue_type, lbound, ubound, paylist[0], paylist[1])
            ...
            if self.retrieved_data is not None:
                if len(self.retrieved_data) > 0 and (
                    ...
                    if issue_type == "obscene language":
                        self.create_table_column("Payload, Description, Banned Word (If Present), Check")
                        for i, dt in enumerate(self.retrieved_data):
                            self.treeview_insert_data(i + 1, [dt[0], dt[1], dt[2], dt[3], ""])
                    else :
    ...
    def retrieveData(self):
        try:
            ...
            if self.isDataValidated:
                if hasattr(self, 'importadder'): self.importadder["state"] = "disabled"
                ...
                if confirm:
                    ...
                    if st:
                        ...
                        if hasattr(self, 'obsceneexportbtn1'): self.obsceneexportbtn1["state"] = "normal"
                        if hasattr(self, 'importadder'): self.importadder["state"] = "disabled"
                    else:
                        self.toggleInputField("normal")
                        self.resetbtn["state"] = "normal"
    ...
    def adderandremoverlist(self, file):
        """ Convert the excel data into userfriendly List Data """

        try:
            logger.debug("Excel File %s", str(file))
            
            work_book = xlrd.open_workbook(file)
            
            adder_list = []            
            adder_sheet = work_book.sheet_by_index(0)
            adder_rows = adder_sheet.nrows
    
            for i in range(1, adder_rows):
                addwords = adder_sheet.cell_value(i, 0)
                if addwords.strip() != "":
                    adder_list.append(addwords)
            logger.debug(adder_list)

            remover_list = []
            remover_sheet = work_book.sheet_by_index(1)
            remover_rows = remover_sheet.nrows

            for i in range(1, remover_rows):
                removewords = remover_sheet.cell_value(i, 0)
                if removewords.strip() != "":
                    remover_list.append(removewords)
            logger.debug(remover_list)
            
            return [adder_list,remover_list]
        except:
    ...
    def exportData(self):
        try:
            if bool(self.data_tree.get_children()):
                ...
                messagebox.showwarning("Warning!!", "No Data to Export!!!", parent=self.querybuilderwin)
        except:
            logger.error("Below Exception occurred\n", exc_info=True)
    ...
    def exportDataList(self, filename):
        try:
            ...
            table_columns = ["Paylodas", "Description", "Is Banned Word Present", "Check", ]
    ...
    def openexcelfile(self):
        try:
            types = (("Excel Files", "*.xlsx *.xls *.xlsm"),
                     ("All Files", "*.*"))
            self.querybuilderwin.excelfile = excelfile = filedialog.askopenfilename(
                initialdir=BASE_SCRIPT_PATH, title="Select Excel File", filetypes=types
            )
            if self.querybuilderwin.excelfile and self.querybuilderwin.excelfile != "" :
                logger.debug("Selected Excel File: " +
                             self.querybuilderwin.excelfile)
                self.varexcelfile.set(self.querybuilderwin.excelfile)
        except:
            logger.error("Below Exception occurred\n", exc_info=True)
    ...
    def changecheckbox(self, val):
        try:
            ...
            for child in self.checkframe.winfo_children():
                if child.winfo_ismapped():
                    child.grid_forget()
            if hasattr(self, 'obsceneexportbtn1'): self.obsceneexportbtn1.pack_forget()
            if hasattr(self, 'importadder'): self.importadder.pack_forget()
            if hasattr(self, 'excelquerylabel'): self.excelquerylabel.pack_forget()
            if val.strip().lower() == "form processing":
                ...
                self.obsceneexportbtn1 = ttk.Button(self.smallbtnframe, text="Obscene Export Test Button1", style="smallbuttondesign.TButton", 
                                                    command=self.exportData
                                                    #command = lambda: print("Obscene Export Button1 was clicked")
                                                    ,)
                self.obsceneexportbtn1["state"] = "disabled"
                self.obsceneexportbtn1.pack(side="left", anchor="w", padx=3, pady=0)

                self.importadder = ttk.Button(self.smallbtnframe, text="Import Adder", style="smallbuttondesign.TButton", 
                                                    command = self.openexcelfile
                                                    ,)
                self.importadder.pack(side="left", anchor="w", padx=3, pady=0)

                self.excelquerylabel = ttk.Label(self.smallbtnframe, text="Browse & Select Excel File..", textvariable=self.varexcelfile, font=(FONT_NAME, FONT_SIZE - 2),
                                             )
                self.excelquerylabel.pack(side="left", anchor="w", padx=3, pady=0)
                #print("looped")
