        ...
        try:
            ...
            def prepare_and_retrv_data(qb_link, p_path, props, r_name, d_type):
                ...
                if r_name.lower() == "page report":
                    ...
                    if d_type != '--select type--':
                        types = "&property="+str(quote("jcr:content/metadata/dc:format",safe=''))+"&property.operation=like&property.value="+str(quote("%"+str(d_type)+"%",safe=''))
                    filter_data = "p.limit=" + str(self.wfoperationdata.get("limit result", 100)) + "&path="+ str(quote(p_path,safe='')) +\
                                    types+"&type="+quote("dam:Asset",safe='')+"&p.hits=selective"
                elif r_name.lower() == "published dam assets report":
                    if d_type != '--select type--':
                        types = "&2_property="+str(quote("jcr:content/metadata/dc:format",safe=''))+"&2_property.operation=like&2_property.value="+str(quote("%"+str(d_type)+"%",safe=''))
                    filter_data = "p.limit=" + str(self.wfoperationdata.get("limit result", 100)) + "&path="+ str(quote(p_path,safe='')) +\
            ...
            if territory.startswith('/content/'):
                full_path = territory
            else:
