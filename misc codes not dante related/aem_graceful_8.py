...
def monitor_log():
    ...
    # Matches a line where "bundle" is followed by any of: uninstalled, deactivated, removed, or stopped.
    regex_bundle_uninstalled = re.compile(
        r"\bbundle\b.*\b(?:uninstalled|deactivated|removed|stopped)\b", re.IGNORECASE
    )

    # Matches a line where "framework" is followed by either "stopped" or "shutdown".
    regex_framework_stopped = re.compile(
        r"\bframework\b.*\b(?:stopped|shutdown)\b", re.IGNORECASE
    )

    # Matches a line containing "forced shutdown" or variant terms like "aborted", "terminated", or "system halt".
    regex_forced_shutdown = re.compile(
        r"\b(?:forced shutdown|aborted|terminated|system halt)\b", re.IGNORECASE
    )

    # Matches a line containing "unresolved", "unsatisfied", "missing dependency", or "dependency error".
    regex_unresolved = re.compile(
        r"\b(?:unresolved|unsatisfied|missing dependency|dependency error)\b", re.IGNORECASE
    )
